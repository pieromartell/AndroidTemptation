...
class AdapterFilterCategory(context: Context?, private val listcategory:List<Category>?):BaseAdapter() {

    private val layoutInflater: LayoutInflater
    private var listaFiltado:List<Category>? = null

    init {
        layoutInflater =  LayoutInflater.from(context)
        listaFiltado = listcategory
    }

    fun filter(texto:String){
        listaFiltado = if (texto.isEmpty()){
            listcategory
        }else{
            listcategory?.filter {
                it.name_cat!!.toString().lowercase().contains(texto.lowercase()) || it.idcat!!.toString().contains(texto)
            }
        }
    ...
    override fun getCount(): Int {
        return listaFiltado!!.size
    }

    override fun getItem(p0: Int): Any {
        return listaFiltado!![p0]
    }
    ...
    override fun getView(p0: Int, p1: View?, p2: ViewGroup?): View {
        var vista = p1
        val objcategory =  listaFiltado!![p0]
        if(vista == null){
